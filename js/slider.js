...
function addImagesToSlider(data) {
    ...
    const categorySliderCard = document.querySelectorAll(
        ".categorySlider__card"
    );
    categorySliderCard.forEach((categoryCard) => {
        const imageID = categoryCard.getAttribute("data-imageID");
        ...
        for (const object of data) {
            ...
            if (imageID == objectId) {
                console.log(
                    `the element ${imageID} is connected to the image ${objectId}`
                );
                // getting the img in the card
                const categorySliderImage = categoryCard.querySelector(
                    ".categorySlider__image"
                ); //categoryCard instead of document -> finds the first one in this section
...
function moveSlider(e) {
    //e = event the call comes from (either arrowPrev or arrowNext)
    ...
    if (e.currentTarget.classList.contains("categorySlider__card")) {
        const cardID = e.currentTarget.getAttribute("data-imageID");
        cardInFocus = parseInt(cardID);
    }

    if (e.currentTarget === sliderArrowPrev) {
        if (cardInFocus > 1) {
            //don't move, if there are no cards left
            cardInFocus -= 1;
        }
    }
    ...
    const newFocus = document.querySelector(
        `.categorySlider__card[data-imageID="${cardInFocus}"]`
    );
    ...
    const currentPage = parseInt(newFocus.getAttribute("data-imageID"));
 ...
 * change content of hero section
 */
...
function adaptHeroSection(data) {
    ...
    const heroDescriptionMobile = document.querySelector(
        ".heroSection__description--mobile"
    );
    const heroImageMobile = document.querySelector(
